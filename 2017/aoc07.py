...
class BaumKnoten:
    ...
    @staticmethod
    def from_name(name: str) -> 'BaumKnoten':
        res = BaumKnoten.all_.get(name)
        if res is None:
            res = BaumKnoten(name)
            BaumKnoten.all_[name] = res
        return res

    def __repr__(self) -> str:
        if self.parent is None:
            return '({0} None)'.format(self.name)
        return '({0} Obj)'.format(self.name)
...
def part1() -> None:
    ...
    for line in INPUT.splitlines():
        ...
        parent = BaumKnoten.from_name(parent_name)

        has_name_list = m.lastindex == 3
        if has_name_list:
            name_list = [_.strip() for _ in m.group(4).split(",")]
            for child_name in name_list:

                child = BaumKnoten.from_name(child_name)
                child.parent = parent

    for bk in BaumKnoten.all_.values():
        if bk.parent is None:
            print(bk.name, "has no parent")


class Part2SolverState(Enum):
    ParentOfAllBalanced = auto()
    Other = auto()


class BessererBaumKnoten:
    alle = dict()

    def __init__(self, n: str):
        self._name = n
        self.weight = None  # only the specific weight of this tree node
        self._recursive_weight = None
        self._all_children_balanced = None
        self._first_child = None
        self._next_sibling = None

    def __repr__(self) -> str:
        return 'w {0} rw {1}'.format(str(self.weight).rjust(6), str(self._recursive_weight).rjust(6))

    @staticmethod
    def from_name(n: str) -> 'BessererBaumKnoten':
        res = BessererBaumKnoten.alle.get(n)
        if res is None:
            res = BessererBaumKnoten(n)
            BessererBaumKnoten.alle[n] = res
        return res

    def addChild(self, c: 'BessererBaumKnoten') -> None:
        if self._first_child is None:
            self._first_child = c
        else:
            self._first_child._addSibling(c)

    def _addSibling(self, c: 'BessererBaumKnoten') -> None:
        add_here = self
        while add_here._next_sibling is not None:
            add_here = add_here._next_sibling
        add_here._next_sibling = c

    def _immediate_chidren(self) -> 'BessererBaumKnoten':
        walk = self._first_child
        while walk is not None:
            yield walk
            walk = walk._next_sibling

    def calc_all_recursive(self) -> None:
        if self._recursive_weight is not None:
            return

        s = self.weight
        all_children_weights = set()
        for c in self._immediate_chidren():
            c.calc_all_recursive()
            s += c._recursive_weight
            all_children_weights.add(c._recursive_weight)

        self._recursive_weight = s
        self._all_children_balanced = len(all_children_weights) == 1

    def _child_with_unique_weight(self) -> 'BessererBaumKnoten':
        weight_counts = collections.Counter()
        weight_map = dict()

        for c in self._immediate_chidren():
            weight_counts[c._recursive_weight] += 1
            weight_map[c._recursive_weight] = c
    ...
    def recursive_solve_part_2(self) -> None:
        def walk_(bbk: 'BessererBaumKnoten') -> Part2SolverState:
            if bbk._all_children_balanced:
                return Part2SolverState.ParentOfAllBalanced

            solver_state = walk_(bbk._child_with_unique_weight())

            if solver_state == Part2SolverState.ParentOfAllBalanced:
                for c in bbk._immediate_chidren():
                    print(repr(c))

            return Part2SolverState.Other
        walk_(self)
...
def part2() -> None:
    ...
    for line in INPUT.splitlines():
        ...
        parent_name = m.group(1)
        parent_weight = int(m.group(2))
        parent = BessererBaumKnoten.from_name(parent_name)
        ...
        if has_name_list:
            name_list = [_.strip() for _ in m.group(4).split(",")]
            for child_name in name_list:
                child = BessererBaumKnoten.from_name(child_name)
                parent.addChild(child)

    root = BessererBaumKnoten.from_name('vgzejbd')
    root.calc_all_recursive()
    root.recursive_solve_part_2()
