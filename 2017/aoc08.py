...
def part1() -> None:
    ...
    regex = re.compile(r'(\w+)\s(inc|dec)\s([0123456789-]+)\sif\s(\w+)\s(<|>|<=|>=|!=|==)\s([0123456789-]+)')

    def if_(cmp_reg: str, cmp_op: str, cmp_val: int) -> bool:
        if cmp_op == "<":
            return registers[cmp_reg] < cmp_val
        elif cmp_op == "<=":
            return registers[cmp_reg] <= cmp_val
        elif cmp_op == ">":
            return registers[cmp_reg] > cmp_val
    ...
    for line in INPUT.splitlines():
        ...
        change_reg = match.group(1)
        op = match.group(2)
        change = int(match.group(3))
        cmp_reg = match.group(4)
        cmp_op = match.group(5)
        cmp_val = int(match.group(6))

        if if_(cmp_reg, cmp_op, cmp_val):
            if op == "inc":
                registers[change_reg] += change
            elif op == "dec":
                registers[change_reg] -= change
            else:
                raise Exception()
