...
def do_part1_reverses(values: List[int], length_list: List[int], start: int, skip: int) -> Tuple[List[int], int, int]:
...
def part1() -> None:
    skip = 0
    start = 0
    (values, _, _) = do_part1_reverses(list(range(256)), [int(_) for _ in INPUT.split(",")], start, skip)

    answer = values[0] * values[1]
    print(f'solution for part 1 {answer}')
...
def part2() -> None:
    ...
    length_list = [int(_) for _ in INPUT.encode("ASCII")] + [17, 31, 73, 47, 23]

    for _ in range(64):
        (values, start, skip) = do_part1_reverses(values, length_list, start, skip)

    xored = []
    for block_id in range(16):
        xored.append(functools.reduce(operator.xor, values[block_id * 16:(block_id + 1) * 16]))

    hex_ = bytes(xored).hex()
    print(f'part 2 solution is {hex_}')
