...
impl WakeynessChange {
    pub fn from_str_with_dt(awake: bool, swd: StrWithDt) -> WakeynessChange {
        WakeynessChange {
            year: swd.year,
            month: swd.month,
            day: swd.day,
    ...
    pub fn same_day(&self, other: &WakeynessChange) -> bool {
        self.year == other.year && self.month == other.month && self.day == other.day
    }
}

struct GuardStartEnds {
    no: i32,
    changes: Vec<WakeynessChange>,
}

impl GuardStartEnds {
    pub fn from_guard_no(no: i32) -> GuardStartEnds {
        GuardStartEnds {
            no,
            changes: Vec::new(),
        }
    ...
    pub fn add_waking(&mut self, when: StrWithDt) {
        self.changes.push(WakeynessChange::from_str_with_dt(true, when));
    }

    pub fn add_sleeping(&mut self, when: StrWithDt) {
    ...
    pub fn asleep_duration(&self) -> u32 {
        ...
        for change in i {
            if change.awake {
                if !last_change.awake {
                    if last_change.hour != 0 {
                        panic!();
                    }
...
fn parse() -> Vec<StrWithDt> {
    ...
    let mut sortable: Vec<StrWithDt> = INPUT.iter().map(|l| re.captures(l).unwrap()).map(|cap| StrWithDt {
...
fn part1(data: Vec<StrWithDt>) -> i32 {
    ...
    for e in data {
        if e.s.contains("begins shift") {
            guard_no = guard_no_re.captures(&e.s).unwrap()[1].parse().unwrap();
            guards.entry(guard_no).or_insert(GuardStartEnds::from_guard_no(guard_no)).add_waking(e);
        } else if e.s.contains("wakes up") {
            if guard_no == -1 { panic!(); }
            guards.entry(guard_no).or_insert(GuardStartEnds::from_guard_no(guard_no)).add_waking(e);
        } else if e.s.contains("falls asleep") {
            if guard_no == -1 { panic!(); }
            guards.entry(guard_no).or_insert(GuardStartEnds::from_guard_no(guard_no)).add_sleeping(e);
        } else {
            panic!(e.s);
        }
    }
